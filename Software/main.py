...
# LCD Display (SPI 0) - Physical Pins 16, 17, 18, 19, 20
spi = machine.SPI(0, baudrate=24000000, mosi=Pin(19), sck=Pin(18))

# HARD RESET LCD: This fixes the "Faded/Washed out" screen issue
...
btn_joy = Pin(22, Pin.IN, Pin.PULL_UP) # Primary Button for Start/Setup
...
def save_highscore(val):
    try:
        with open("highscore.txt", "w") as f: f.write(str(val))
    except: pass
    ...
    for b in bricks:
        if b[3] and b[0] < bx < b[0]+38 and b[1] < by < b[1]+10:
            ...
            if not any(bk[3] for bk in bricks):
                wipe_ball(); play_sound(1500, 100); score += 100
                if score > high_score: 
                    high_score = score
